...
// 城市名 -> 经纬度（Open‑Meteo geocoding），仅挑中国结果
async function geocodeCity(name: string): Promise<{
  ...
  const cnList = list.filter((it) => String(it?.country_code || '').toUpperCase() === 'CN')
  if (cnList.length === 0) throw new Error('未找到该城市（中国境内）')

  // 精确优先：名称或 admin1 等于输入
  const n = name.trim()
  const exact =
    cnList.find((it) => String(it?.name || '').trim() === n) ||
    cnList.find((it) => String(it?.admin1 || '').trim() === n)

  const item = exact || cnList[0]
  return {
    lat: item.latitude,
    lon: item.longitude,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Resp>) {
  try {
    ...
    if (lat && lon) {
      ...
      // 你前端传的是“省/直辖市名”，这里按“省级选择”处理
      const g = await geocodeCity(String(city))
      latNum = g.lat
      lonNum = g.lon
      geo = {
        city: '', // 省级选择时不展示 city，避免“北京 · 重庆市”观感
    ...
    const climateData = {
      ...
      city: provinceNorm // 用省/直辖市名命中 provinceToRegion
    }
    ...
    return res.status(200).json({
      ...
      city: geo.city || geo.province, // 前端已做同名折叠展示
      season,
      weather: {
        temp: wx.temp ?? null,
