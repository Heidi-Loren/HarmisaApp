...
type Ok = { ok: true; deviceId: string; snapshot: Snapshot }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Resp>) {
  ...
  try {
    const deviceId = String((req.method === 'POST' ? (req.body as any)?.deviceId : req.query?.deviceId) || '')
    if (!deviceId) return res.status(400).json({ error: 'deviceId required' })

    // —— 体质：取该 device_id 最新一条
    ...
    // —— upsert 到 device_profiles（以 device_id 为唯一键）
    ...
    return res.status(200).json({ ok: true, deviceId, snapshot })
  } catch (e: any) {
